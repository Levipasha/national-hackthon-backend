...
router.post('/auth/otp-verify', async (req, res) => {
    ...
    if (user.teamId && user.paymentStatus !== 'paid' && user.role !== 'team-leader' && user.teamRole !== 'leader') {
        ...
        const leader = team ? await db_1.Users.findOne({ id: team.leaderId }) : null;
...
router.get('/public/problem-assignments', async (req, res) => {
    try {
        const teams = await db_1.Teams.find({});
        const problems = await db_1.ProblemDb.find({});
        const users = await db_1.Users.find({});
...
router.get('/public/participants', async (req, res) => {
    try {
        ...
        const allUsers = await db_1.Users.find(u => u.role !== 'admin' &&
            ...
            (u.paymentStatus === 'paid' || u.paymentStatus === 'submitted'));
...
router.post('/teams/update-problem-sno', exports.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        let team = null;
        if (user.teamId) {
            team = await db_1.Teams.findOne({ id: user.teamId });
        }
        if (!team) {
            team = await db_1.Teams.findOne(t => t.leaderId === userId || (Array.isArray(t.members) && t.members.includes(userId)));
        }
        if (!team) {
        ...
        const problems = await db_1.ProblemDb.find({});
        const matchedProblem = problems.find(p => String(p.sno || '').trim().toLowerCase() === inputSno.toLowerCase() ||
        ...
        await db_1.Teams.updateOne(team.id, updateData);
        if (matchedProblem) {
            for (const p of problems) {
                if (Array.isArray(p.assignedTo) && p.assignedTo.includes(team.id)) {
                    const updatedAssigned = p.assignedTo.filter((tId) => tId !== team.id);
                    await db_1.ProblemDb.updateOne(p.id, { assignedTo: updatedAssigned });
                }
            }
            ...
            if (!currentAssigned.includes(team.id)) {
                await db_1.ProblemDb.updateOne(matchedProblem.id, { assignedTo: [...currentAssigned, team.id] });
            }
        }
        if (team.members && Array.isArray(team.members)) {
            for (const mId of team.members) {
                await db_1.Users.updateOne(mId, { problemSno: updateData.problemSno });
            }
        }
        if (team.leaderId) {
            await db_1.Users.updateOne(team.leaderId, { problemSno: updateData.problemSno });
        }
        const updatedTeam = await db_1.Teams.findOne({ id: team.id });
        return res.json({
            success: true,
...
router.get(['/admin/stats', '/admin/overview'], exports.authenticateToken, exports.requireAdmin, async (req, res) => {
    try {
        const allUsers = (await db_1.Users.find(u => u && !u.hidden && u.email?.toLowerCase() !== 'vamshi.c2002@gmail.com')) || [];
        const allTeams = (await db_1.Teams.find()) || [];
...
router.get('/admin/participants', exports.authenticateToken, exports.requireAdmin, async (req, res) => {
    ...
    let list = await db_1.Users.find(u => u && u.role !== 'admin' && !u.hidden && u.email?.toLowerCase() !== 'vamshi.c2002@gmail.com');
...
router.put('/admin/teams/:id/problem-sno', exports.authenticateToken, exports.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (team.members && Array.isArray(team.members)) {
            for (const mId of team.members) {
                await db_1.Users.updateOne(mId, { problemSno: inputSno });
            }
        }
        if (team.leaderId) {
            await db_1.Users.updateOne(team.leaderId, { problemSno: inputSno });
        }
...
router.get('/admin/export-csv', exports.authenticateToken, exports.requireAdmin, async (req, res) => {
    const users = await db_1.Users.find(u => u && u.role !== 'admin' && !u.hidden && u.email?.toLowerCase() !== 'vamshi.c2002@gmail.com');
    const allTeams = await db_1.Teams.find();
    const teamMap = {};
...
router.get('/user/problem-statements', exports.authenticateToken, async (req, res) => {
    ...
    if (!userTeamId) {
        const userTeam = await db_1.Teams.findOne(t => t.leaderId === user.id || (Array.isArray(t.members) && t.members.includes(user.id)));
        if (userTeam)
